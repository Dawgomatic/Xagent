...
type SkillCatalogEntry struct {
	...
	Source      string   `json:"source"` // "builtin", "community", "user"
...
type AutoDiscoverer struct {
	catalogPath    string // path to skill catalog JSON
...
// catalogPath points to the skill catalog JSON (generated by skill_converter.py).
func NewAutoDiscoverer(workspace, catalogPath string) *AutoDiscoverer {
	ad := &AutoDiscoverer{
...
func (ad *AutoDiscoverer) loadCatalog() {
	data, err := os.ReadFile(ad.catalogPath)
	if err != nil {
		// Try alternative locations
		altPaths := []string{
			filepath.Join(filepath.Dir(ad.installDir), "..", "reference", "openclaw-skills", "catalog.json"),
			filepath.Join(filepath.Dir(ad.installDir), "skill_catalog.json"),
		}
		...
		if err != nil {
			logger.DebugCF("skills", "No skill catalog found (auto-discovery disabled)", nil)
			return
		}
	}

	if err := json.Unmarshal(data, &ad.catalog); err != nil {
		logger.WarnCF("skills", "Failed to parse skill catalog",
			map[string]interface{}{"error": err.Error()})
	}

	logger.InfoCF("skills", "Skill catalog loaded",
