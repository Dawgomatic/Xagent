	...
	if providers, ok := getMap(data, "providers"); ok {
		for name, val := range providers {
			...
			if !supportedProviders[name] {
				if apiKey != "" || apiBase != "" {
					warnings = append(warnings, fmt.Sprintf("Provider '%s' not supported in Xagent, skipping", name))
				}
				continue
			}
	...
	if channels, ok := getMap(data, "channels"); ok {
		for name, val := range channels {
			...
			if !supportedChannels[name] {
				warnings = append(warnings, fmt.Sprintf("Channel '%s' not supported in Xagent, skipping", name))
				continue
			}
...
func rewriteWorkspacePath(path string) string {
	path = strings.Replace(path, ".openclaw", ".xagent", 1)
	return path
}
