...
func Run(opts Options) (*Result, error) {
	...
	xagentHome, err := resolveXagentHome(opts.XagentHome)
	if err != nil {
		return nil, err
	}
	...
	actions, warnings, err := Plan(opts, openclawHome, xagentHome)
	if err != nil {
		return nil, err
	}

	fmt.Println("Migrating from OpenClaw to Xagent")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	result := Execute(actions, openclawHome, xagentHome)
	result.Warnings = warnings
	return result, nil
}

func Plan(opts Options, openclawHome, xagentHome string) ([]Action, []string, error) {
	var actions []Action
	var warnings []string
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Source:      configPath,
				Destination: filepath.Join(xagentHome, "config.json"),
				Description: "convert OpenClaw config to Xagent format",
			})
	...
	if !opts.ConfigOnly {
		...
		dstWorkspace := resolveWorkspace(xagentHome)
...
func Execute(actions []Action, openclawHome, xagentHome string) *Result {
	result := &Result{}

	for _, action := range actions {
		switch action.Type {
		case ActionConvertConfig:
			if err := executeConfigMigration(action.Source, action.Destination, xagentHome); err != nil {
...
func executeConfigMigration(srcConfigPath, dstConfigPath, xagentHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing Xagent config: %w", err)
		}
...
func resolveXagentHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	if envHome := os.Getenv("XAGENT_HOME"); envHome != "" {
		return expandHome(envHome), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
	...
	return filepath.Join(home, ".xagent"), nil
}
