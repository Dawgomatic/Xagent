...
func TestConvertConfig(t *testing.T) {
	...
	t.Run("unsupported provider warning", func(t *testing.T) {
		...
		if warnings[0] != "Provider 'deepseek' not supported in Xagent, skipping" {
	...
	t.Run("unsupported channel warning", func(t *testing.T) {
		...
		if warnings[0] != "Channel 'email' not supported in Xagent, skipping" {
			t.Errorf("unexpected warning: %s", warnings[0])
		}
	})
	...
	t.Run("agent defaults", func(t *testing.T) {
		...
		if cfg.Agents.Defaults.Workspace != "~/.xagent/workspace" {
			t.Errorf("Workspace = %q, want %q", cfg.Agents.Defaults.Workspace, "~/.xagent/workspace")
		}
	})
...
func TestRewriteWorkspacePath(t *testing.T) {
	tests := []struct {
		...
		{"default path", "~/.openclaw/workspace", "~/.xagent/workspace"},
...
func TestRunDryRun(t *testing.T) {
	...
	xagentWs := filepath.Join(xagentHome, "workspace")
	if _, err := os.Stat(filepath.Join(xagentWs, "SOUL.md")); !os.IsNotExist(err) {
		t.Error("dry run should not create files")
	}
	if _, err := os.Stat(filepath.Join(xagentHome, "config.json")); !os.IsNotExist(err) {
...
func TestRunFullMigration(t *testing.T) {
	...
	soulData, err := os.ReadFile(filepath.Join(xagentWs, "SOUL.md"))
	if err != nil {
	...
	agentsData, err := os.ReadFile(filepath.Join(xagentWs, "AGENTS.md"))
	if err != nil {
	...
	memData, err := os.ReadFile(filepath.Join(xagentWs, "memory", "MEMORY.md"))
	if err != nil {
	...
	xagentConfig, err := config.LoadConfig(filepath.Join(xagentHome, "config.json"))
	if err != nil {
		t.Fatalf("loading Xagent config: %v", err)
	}
	if xagentConfig.Providers.Anthropic.APIKey != "sk-ant-migrate-test" {
		t.Errorf("Anthropic.APIKey = %q, want %q", xagentConfig.Providers.Anthropic.APIKey, "sk-ant-migrate-test")
	}
	if xagentConfig.Providers.OpenRouter.APIKey != "sk-or-migrate-test" {
		t.Errorf("OpenRouter.APIKey = %q, want %q", xagentConfig.Providers.OpenRouter.APIKey, "sk-or-migrate-test")
	}
	if !xagentConfig.Channels.Telegram.Enabled {
		t.Error("Telegram should be enabled")
	}
	if xagentConfig.Channels.Telegram.Token != "tg-migrate-test" {
		t.Errorf("Telegram.Token = %q, want %q", xagentConfig.Channels.Telegram.Token, "tg-migrate-test")
	}
