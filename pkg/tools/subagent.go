...
// SWE100821: Concurrency limit to prevent runaway subagent spawning
...
func (sm *SubagentManager) Spawn(ctx context.Context, task, label, originChannel, originChatID string, callback AsyncCallback) (string, error) {
	// SWE100821: Enforce concurrency limit to prevent resource exhaustion
	select {
	case sm.sem <- struct{}{}:
		// Acquired slot
	default:
		return "", fmt.Errorf("subagent limit reached (%d concurrent); try again later", maxConcurrentSubagents)
	}
