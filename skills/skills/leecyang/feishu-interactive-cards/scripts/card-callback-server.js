...
function loadFeishuConfig() {
  try {
    ...
    console.error(' 无法读取飞书配置:', error.message);
  }
...
function loadGatewayConfig() {
  try {
    ...
    console.log(' 无法读取 OpenClaw 配置:', error.message);
  }
...
console.log(' Gateway Token:', GATEWAY_TOKEN ? '已配置 ' : '未配置 ');
console.log(' Gateway 集成:', GATEWAY_ENABLED ? '已启用 ' : '已禁用 ');
console.log('');
...
async function sendToGateway(callbackData) {
  ...
  try {
    ...
    if (error.code === 'ECONNREFUSED') {
      ...
      console.log(' Gateway Token 验证失败');
    } else {
      console.log(' 发送到 Gateway 失败:', error.message);
    }
  }
}
  ...
  'card.action.trigger': async (data) => {
    ...
    switch (actionValue?.action) {
              ...
              elements: [
                ...
                },
                {
                  ...
                  actions: [
                    ...
                    {
                      tag: "button",
                      text: { content: " 查看天气", tag: "plain_text" },
                      type: "default",
        ...
        response = {
          ...
          card: {
            ...
            data: {
              config: { wide_screen_mode: true },
              header: {
                title: { content: " 今日天气", tag: "plain_text" },
                template: "blue"
              },
              elements: [
                {
              ...
              elements: [
                ...
                },
                {
                  ...
                  actions: [
                    {
                      tag: "button",
                      text: { content: " 下一题", tag: "plain_text" },
                      type: "primary",
      ...
      case 'clear_completed_todos':
        ...
        console.log(' 清除已完成任务:', clearedCount, '个');
        ...
        if (remainingTodos.length === 0) {
          ...
          response = {
            toast: {
              type: 'success',
              content: ` 已清除 ${clearedCount} 个已完成任务`
            },
            card: {
              type: 'raw',
              data: {
      ...
      default:
        console.log(` 未知操作: ${JSON.stringify(actionValue)}`);
        response = {
          toast: {
            type: 'info',
            content: '操作成功'
