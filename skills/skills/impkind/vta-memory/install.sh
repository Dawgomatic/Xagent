...
if [ "$1" = "--with-cron" ]; then
  ...
  if ! command -v openclaw &> /dev/null; then
    echo "  'openclaw' not in PATH. Add these cron jobs manually:"
    echo ""
    ...
    openclaw cron add --name vta-decay \
      --cron '0 4,12,20 * * *' \
      --session isolated \
      --agent-turn " Run drive decay: Run $SKILL_DIR/scripts/decay-drive.sh and report new drive level" 2>/dev/null && echo "    Created" || echo "     Already exists"
    
    echo "   Creating vta-encoding..."
    openclaw cron add --name vta-encoding \
      --cron '45 0,3,6,9,12,15,18,21 * * *' \
      --session isolated \
      --agent-turn "Run VTA reward encoding: 1) Run preprocess-rewards.sh 2) Read encode-rewards.md 3) Log rewards found 4) Resolve fulfilled anticipations 5) Sync state 6) Update watermark" 2>/dev/null && echo "    Created" || echo "     Already exists"
  fi
fi
