...
- `disableCSRFCheck`: Disable all CSRF protection including origin header validation and Fetch Metadata checks ( security risk)
- `disableOriginCheck`: Disable URL validation for `callbackURL`, `redirectTo`, and other redirect URLs ( security risk)
