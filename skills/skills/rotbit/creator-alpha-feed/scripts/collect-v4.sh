...
if curl -s --max-time 30 "$API_URL" -o "$HN_FILE" 2>/dev/null && jq -e '.hits' "$HN_FILE" > /dev/null 2>&1; then
    ...
    if [[ $ITEM_COUNT -gt 0 ]]; then
        ...
        
        {
            ...
            echo "$ITEMS" | jq -r '.[] | 
                "### " + .title + "\n" +
                "-   " + (.points | tostring) + " points |  " + (.comments | tostring) + " |  @" + .author + "\n" +
                "-  [原文](" + .url + ") | [HN](" + .hn_url + ")\n" +
                "---\n"
            '
        } >> "$MARKDOWN_FILE"
...
for SUBREDDIT in "ArtificialIntelligence" "singularity"; do
    ...
    if curl -s --max-time 25 \
        ...
        if [[ $ITEM_COUNT -gt 0 ]]; then
            ...
            echo "$ITEMS" | jq -r '.[] | 
                "- **" + .title + "**\n" +
                "   " + (.upvotes | tostring) + " |  " + (.comments | tostring) + " | [链接](" + .url + ")\n"
            ' >> "$MARKDOWN_FILE"
...
EOF
    ...
    log "    已跳过 (使用 --twitter 启用)"
    {
        echo "##  Twitter"
        echo ""
        echo " **已跳过** - 使用 \`--twitter\` 参数启用browser收集"
        echo ""
