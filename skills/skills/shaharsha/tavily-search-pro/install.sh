...
pip install --break-system-packages --quiet tavily-python 2>/dev/null || {
    echo "  pip install failed, trying without --break-system-packages..."
...
if [ -z "${TAVILY_API_KEY:-}" ]; then
    echo "  TAVILY_API_KEY not set. Set it in OpenClaw config before using."
else
...
if python3 "$SCRIPT_DIR/lib/tavily_search.py" --help >/dev/null 2>&1; then
    ...
    echo "  Smoke test failed - check Python dependencies."
    exit 1
fi
