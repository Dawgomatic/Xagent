...
if [ ! -f "$WORKSPACE_DIR/.gitignore" ]; then
    ...
    echo "     .gitignore exists, skipping (see templates/gitignore for reference)"
fi
...
if [ ! -f "$WORKSPACE_DIR/.env" ]; then
    ...
    echo "     .env exists, preserving (see templates/env.example for reference)"
fi
...
if [ -f "$WORKSPACE_DIR/AGENTS.md" ]; then
    ...
    if grep -q "##  Artifact Workflow" "$WORKSPACE_DIR/AGENTS.md"; then
        echo "     AGENTS.md already has Workspace Pro sections, skipping"
    else
    ...
    echo "     AGENTS.md not found, skipping (create workspace first)"
fi
...
if [ -f "$WORKSPACE_DIR/TOOLS.md" ]; then
    if grep -q "### Network Security - Approved Domains" "$WORKSPACE_DIR/TOOLS.md"; then
        echo "     TOOLS.md already has network security section, skipping"
    else
        cat "$SKILL_DIR/templates/TOOLS-additions.md" >> "$WORKSPACE_DIR/TOOLS.md"
        echo "    TOOLS.md updated"
    fi
else
    echo "     TOOLS.md not found, skipping (create workspace first)"
fi
