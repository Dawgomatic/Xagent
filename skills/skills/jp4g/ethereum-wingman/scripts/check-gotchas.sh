...
if [ -n "$INFINITE_APPROVALS" ]; then
    echo "  POTENTIAL ISSUE: Infinite approvals found:" >&2
...
if [ -n "$TX_ORIGIN" ]; then
    echo "  POTENTIAL ISSUE: tx.origin found (phishing vulnerability):" >&2
    echo "$TX_ORIGIN" >&2
    echo "" >&2
    ISSUES_FOUND=$((ISSUES_FOUND + 1))
fi
...
if [ -n "$HARDCODED_DECIMALS" ]; then
    echo "  NOTE: Hardcoded 1e18 found - verify these aren't decimal assumptions:" >&2
...
if [ -n "$EXTERNAL_CALLS" ]; then
    echo "  NOTE: External calls found - verify CEI pattern:" >&2
...
if [ -n "$HAS_EXTERNAL_CALLS" ]; then
    for file in $HAS_EXTERNAL_CALLS; do
        if ! grep -q "nonReentrant\|ReentrancyGuard" "$file" 2>/dev/null; then
            echo "  POTENTIAL ISSUE: $file has external calls but no ReentrancyGuard" >&2
            ISSUES_FOUND=$((ISSUES_FOUND + 1))
        fi
    done
fi
...
if [ -n "$DEX_PRICES" ]; then
    echo "  POTENTIAL ISSUE: DEX spot price usage found (flash loan vulnerable):" >&2
    echo "$DEX_PRICES" >&2
    echo "" >&2
    ISSUES_FOUND=$((ISSUES_FOUND + 1))
fi
...
if [ $ISSUES_FOUND -eq 0 ]; then
    ...
    echo "  Found $ISSUES_FOUND potential issues to review" >&2
fi
echo "" >&2
