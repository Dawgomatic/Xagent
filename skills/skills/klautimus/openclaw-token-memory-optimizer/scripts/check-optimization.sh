...
if grep -q '"sessionTarget".*"isolated"' "$CONFIG" 2>/dev/null; then
    ...
    [ "$MAIN" != "0" ] && echo "     Found $MAIN job(s) still in main session"
else
    echo "     No isolated jobs found - background tasks may bloat context"
fi
...
if grep -q '"memorySearch"' "$CONFIG" 2>/dev/null; then
    ...
    if grep -q '"provider".*"local"' "$CONFIG"; then
        ...
        echo "     Using remote embeddings (may incur API costs)"
    fi
else
    echo "     memorySearch not configured - using full history reads"
fi
