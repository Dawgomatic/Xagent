...
try:
    ...
    print(f'     语义记忆存储失败: {str(e)}')
"
        else
            echo "     Baidu Embedding未配置 (缺少API凭据)，跳过语义存储"
        fi
    ...
    search-all)
        ...
        if [ -n "$BAIDU_API_STRING" ] && [ -n "$BAIDU_SECRET_KEY" ]; then
            ...
            echo "  Baidu Embedding未配置 (缺少API凭据)，跳过语义搜索"
        fi
    ...
    status)
        ...
        if [ -n "$BAIDU_API_STRING" ] && [ -n "$BAIDU_SECRET_KEY" ]; then
            ...
            echo "  Baidu Embedding未配置 (缺少API凭据)"
        fi
