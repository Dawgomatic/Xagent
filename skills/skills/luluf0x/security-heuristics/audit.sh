...
if [ -n "$CMD_SUB" ]; then
  echo "  FOUND command substitution:"
...
if [ -n "$CREDS" ]; then
  echo "  FOUND credential references:"
  echo "$CREDS" | head -10
else
...
echo ""
echo "Remember:"
echo "-  warnings need manual review, not automatic rejection"
