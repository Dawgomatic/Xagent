...
if [ ! -d "$OPENCLAW_CONFIG_DIR" ]; then
    echo "    配置目录不存在: $OPENCLAW_CONFIG_DIR"
else
...
echo ""
echo " 创建辅助文件..."
cp "$TEMPLATE_DIR/install-node.sh" "$OUTPUT_DIR/" 2>/dev/null || echo "    install-node.sh 模板不存在"
cp "$TEMPLATE_DIR/check-env.sh" "$OUTPUT_DIR/" 2>/dev/null || echo "    check-env.sh 模板不存在"
cp "$TEMPLATE_DIR/README.md" "$OUTPUT_DIR/" 2>/dev/null || echo "    README.md 模板不存在"
echo "   辅助文件创建完成"
