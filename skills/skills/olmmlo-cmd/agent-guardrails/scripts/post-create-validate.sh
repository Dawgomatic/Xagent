...
if [ -z "$NEW_FUNCS" ]; then
    echo "  No function definitions found in new file."
else
    ...
    for func_name in $NEW_FUNCS; do
        ...
        if [ -n "$EXISTING" ]; then
            echo "    DUPLICATE: '$func_name' already exists!"
            echo "     $EXISTING"
            WARNINGS=$((WARNINGS + 1))
        fi
    done
...
if [ -n "$ESTABLISHED" ]; then
    ...
    if [ "$IMPORTS_FOUND" -eq 0 ]; then
        echo "    WARNING: Does not import from any established module!"
        echo "     Established modules:$ESTABLISHED"
        echo "     Consider importing validated functions instead of reimplementing."
        WARNINGS=$((WARNINGS + 1))
    fi
else
    echo "    No established modules (>5 functions) found in project."
fi
echo ""
...
if [ "$WARNINGS" -gt 0 ]; then
    echo "╔══════════════════════════════════════════════════╗"
    echo "║    $WARNINGS WARNING(S) — Review before proceeding   ║"
    echo "╚══════════════════════════════════════════════════╝"
    exit 1
else
    echo "╔══════════════════════════════════════════════════╗"
