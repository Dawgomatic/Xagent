...
while IFS= read -r file; do
    ...
    if [ -n "$MATCH" ]; then
        echo "    os.getenv() with fallback in $file:"
        echo "$MATCH" | head -3 | sed 's/^/     /'
        ERRORS=$((ERRORS + 1))
    fi
done <<< "$FILES"
...
while IFS= read -r file; do
    ...
    if [ -n "$MATCH" ]; then
        echo "    SQL injection risk in $file:"
        echo "$MATCH" | head -3 | sed 's/^/     /'
        ERRORS=$((ERRORS + 1))
    fi
done <<< "$FILES"
...
while IFS= read -r file; do
    ...
    if [ -n "$MATCH" ]; then
        echo "    Command injection risk in $file:"
        echo "$MATCH" | head -3 | sed 's/^/     /'
        ERRORS=$((ERRORS + 1))
    fi
done <<< "$FILES"
...
if [ -f "package.json" ] && command -v npm >/dev/null 2>&1; then
    ...
    echo "    package.json found but npm not available"
fi
...
if ([ -f "requirements.txt" ] || [ -f "pyproject.toml" ] || [ -d "venv" ]) && command -v pip-audit >/dev/null 2>&1; then
    ...
    echo "    Python project detected but pip-audit not available"
fi
...
if [ ! -f ".gitignore" ]; then
    ...
    if [ ${#MISSING_PATTERNS[@]} -gt 0 ]; then
        echo "    .gitignore missing sensitive patterns: ${MISSING_PATTERNS[*]}"
        ERRORS=$((ERRORS + 1))
    else
