...
if [[ "$(uname)" == "Darwin" ]]; then
  ...
  if command -v freshclam &>/dev/null; then
    ...
    freshclam --quiet 2>/dev/null && echo -e "   Signatures updated" || echo -e "  ${YELLOW}  Signature update failed (may need sudo)${NC}"
  fi
fi
...
if echo "$RESULT" | grep -q '"clean"'; then
  ...
  echo -e "  ${YELLOW}  Scanner returned unexpected result (ClamAV may still be updating signatures)${NC}"
fi
...
if [[ -n "${CRUSTY_API_KEY:-}" ]]; then
  ...
  if bash "$SCRIPT_DIR/scripts/dashboard.sh" heartbeat 2>/dev/null; then
    ...
    echo -e "  ${YELLOW}  Heartbeat failed (dashboard may be unreachable)${NC}"
  fi
  ...
  if bash "$SCRIPT_DIR/scripts/host_audit.sh" 2>/dev/null; then
    ...
    echo -e "  ${YELLOW}  Host audit completed with errors${NC}"
  fi
  ...
  if bash "$SCRIPT_DIR/scripts/scan_file.sh" -r "$SCAN_DIR" 2>/dev/null; then
    ...
    echo -e "  ${YELLOW}  Workspace scan completed with errors${NC}"
  fi
  ...
  if bash "$SCRIPT_DIR/scripts/monitor_agent.sh" 2>/dev/null; then
    ...
    echo -e "  ${YELLOW}  Agent monitor completed with warnings${NC}"
  fi
  ...
  if [[ -f "$SCRIPT_DIR/scripts/clawhub_sync.py" ]]; then
    ...
    if python3 "$SCRIPT_DIR/scripts/clawhub_sync.py" --push 2>/dev/null; then
      ...
      echo -e "  ${YELLOW}  Skill sync failed (non-critical)${NC}"
    fi
  fi
  ...
  echo ""
  echo -e "  ${YELLOW}  No CRUSTY_API_KEY set — running in local-only mode${NC}"
...
if echo "$CRON_CHECK" | grep -q '"all_configured":true'; then
  ...
  echo -e "  ${YELLOW}  ${MISSING_COUNT} scheduled scan(s) not yet configured${NC}"
