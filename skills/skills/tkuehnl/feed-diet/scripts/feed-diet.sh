...
show_help() {
  cat <<'EOF'
    ...
    Feed Diet v0.1.1 — Audit Your Information Diet
  ═══════════════════════════════════════════════════
...
if opinion_count > tutorial_count * 2:
    recs.append(' **Balance opinions with practice:** For every opinion piece, read a tutorial or paper.')
if diversity < 0.5:
    recs.append(' **Diversify:** Try exploring categories outside your comfort zone.')
if top_pct >= 60:
    recs.append(f' **Rebalance:** Your top category is {top_pct:.0f}%. Consider capping any single category at ~40%.')
...
print('---')
print(f'*Feed Diet Weekly Digest v{version} *')
print()
" 2>/dev/null
}
