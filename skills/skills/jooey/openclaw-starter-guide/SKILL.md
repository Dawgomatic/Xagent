...
** 注意**：MiniMax 额度查询 API (`/coding_plan/remains`) 的数据可能不准确（窗口切换后不及时刷新）。判断额度是否可用，最可靠的方法是**发一个真实测试请求**。
...
** 牢记**：`agents.defaults.models.<id>` 只允许 `alias` 字段！一个非法字段 = 全面崩溃。
