...
for file in "${FILES[@]}"; do
  ...
  if [[ "$file" == /* ]] || [[ "$file" == *..* ]] || [[ "$file" == *$'\n'* ]] || [[ "$file" == *$'\0'* ]]; then
    echo "  ${file} — REJECTED (path traversal or absolute path)" >&2
    continue
  fi
  ...
  if [[ "$REAL_PATH" != "${ROOT_DIR}"/* ]]; then
    echo "  ${file} — REJECTED (resolves outside project root)" >&2
    continue
  fi
...
for cred_path in "${ROOT_DIR}/config/credentials.json" "${XDG_CONFIG_HOME:-$HOME/.config}/agentaudit/credentials.json"; do
  if [ -f "$cred_path" ]; then
    ...
    if [ "$PERMS" != "600" ]; then
      echo "  $cred_path has permissions ${PERMS}, fixing to 600"
      chmod 600 "$cred_path"
    fi
  fi
done
