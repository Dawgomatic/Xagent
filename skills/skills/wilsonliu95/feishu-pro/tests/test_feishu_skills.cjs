...
async function testFeishuThreads() {
  ...
  try {
    ...
    // 测试获取话题消息列表
    console.log('\n 测试 listThreadMessages()');
    try {
      // 注意：这个函数需要在话题中才能测试
      console.log('  跳过：需要在已有话题中测试');
