#!/bin/bash
# ContextKeeper Dashboard v0.1.1
...
if [ -L "$CURRENT" ] && [ -f "$CURRENT" ]; then
    ...
    if [ -f "$CURRENT" ]; then
        ...
        if command -v python3 > /dev/null 2>&1; then
            REL_TIME=$(python3 -c "from datetime import datetime; print(f'{(datetime.utcnow() - datetime.fromisoformat(\"$TIMESTAMP\".replace(\"Z\", \"+00:00\"))).seconds // 60}m ago')" 2>/dev/null | sed 's/\[.*\]//g')
        else
            REL_TIME="$TIMESTAMP"
        fi
...
echo "  status                     # View project state"
