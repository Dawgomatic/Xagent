...
[ "$TELEGRAM_DM" = "pairing" ] && echo -e "${GREEN}    Secure (pairing)${NC}" || echo -e "${YELLOW}     Not pairing${NC}"

echo "WhatsApp dmPolicy:  $WHATSAPP_DM"
[ "$WHATSAPP_DM" = "pairing" ] && echo -e "${GREEN}    Secure (pairing)${NC}" || echo -e "${YELLOW}     Using: $WHATSAPP_DM${NC}"

echo "Sandbox scope:      $SANDBOX_SCOPE"
[ "$SANDBOX_SCOPE" = "agent" ] && echo -e "${GREEN}    Explicit agent scope${NC}" || echo -e "${YELLOW}     Using: $SANDBOX_SCOPE${NC}"
...
if [ -d ~/.clawdbot/sessions ]; then
  ...
  echo -e "${BLUE}${NC}  No sessions directory (normal for fresh install)"
fi
...
if [ -d ~/.clawdbot/agents ]; then
  ...
  echo -e "${BLUE}${NC}  No agents directory"
fi
...
if [ -d ~/.clawdbot-backups ]; then
  ...
  echo -e "${BLUE}${NC}  No backups yet (run backup script first)"
fi
