...
# Confirmation
echo -e "${YELLOW}  WARNING: This will overwrite current configuration and workspaces!${NC}"
...
if [ -d ~/code/clawdbot ]; then
  ...
  echo -e "${YELLOW}  Clawdbot repo not found, skipping gateway stop${NC}"
fi
...
if [ -f "$BACKUP_DIR/sessions.tar.gz" ]; then
  ...
  echo -e "${BLUE}  No sessions backup${NC}"
fi
...
if [ -f "$BACKUP_DIR/agents.tar.gz" ]; then
  ...
  echo -e "${BLUE}  No agents backup${NC}"
fi
...
if [ -f "$BACKUP_DIR/credentials.tar.gz" ]; then
  ...
  echo -e "${YELLOW}  No credentials backup${NC}"
fi
...
if [ -f "$BACKUP_DIR/sandboxes.tar.gz" ]; then
  ...
  echo -e "${BLUE}  No sandboxes backup${NC}"
fi
...
if [ -z "$WORKSPACE_DATA" ]; then
  echo -e "${BLUE}  No agents configured${NC}"
else
  RESTORED_COUNT=0
  ...
  while IFS='|' read -r agent_id workspace agent_name; do
    if [ "$workspace" != "none" ]; then
      ...
      if [ -f "$BACKUP_FILE" ]; then
        ...
        echo -e "  ${YELLOW}${NC}  No backup found for '$agent_id' (expected: $BACKUP_FILE)"
      fi
    fi
  done <<< "$WORKSPACE_DATA"
...
if [ -f "$BACKUP_DIR/git-version.txt" ]; then
  ...
  if [[ "$response" =~ ^[Yy]$ ]]; then
    ...
    echo -e "${BLUE}  Keeping current git version${NC}"
  fi
fi
