...
→ If no feedback exists:  WARNING — "No one has used v{X} yet. Why release v{X+1}?"
...
→ Missing English:  WARNING — "Global users can't read this"
→ All present:  PASS
```
...
→ If answer is vague ("minor fixes", "improvements"):  WARNING — "Be specific. What changed?"
...
→ If no kill criteria defined:  WARNING — "Define when to stop: 'If X doesn't happen in Y weeks, shut it down.'"
...
→ Look for contradictions:
  ...
  - "Finish before starting new" + new project while old unfinished = 
...
```
 ...
 WARNING only → Can release, but agent must voice concern clearly.
