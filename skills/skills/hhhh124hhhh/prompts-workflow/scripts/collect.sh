...
报告: $REPORT_FILE" || log "  没有变更需要提交"

git push origin master 2>&1 | tee -a "$LOG_FILE" || log "  Git push 失败或已最新"
...
clawdbot message send \
  ...
  --message "$FEISHU_MESSAGE" >> "$LOG_FILE" 2>&1 || log "  Feishu 通知发送失败"
...
clawdbot message send \
  --channel slack \
  --target D0AB0J4QLAH \
  --message "$SLACK_MESSAGE" >> "$LOG_FILE" 2>&1 || log "  Slack 通知发送失败"
