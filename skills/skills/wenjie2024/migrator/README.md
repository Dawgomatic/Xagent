...
-  **Cross-Generation Compatibility (v1.1.0)**: Supports seamless migration from legacy `~/.clawdbot` to the new `~/.openclaw` standard.
-  **Path Self-healing**: Automatically repairs absolute paths and workspace roots during restoration to ensure zero-config restarts.
...
-  **路径自愈**：在恢复过程中自动修正绝对路径（如 workspace 根目录），确保迁移后无需手动修改配置。
-  **环境感知**：自动捕获宿主机元数据（Node版本、平台、架构），防止目标环境不兼容。
-  **安全加固**：内置路径遍历（Path Traversal）防御，拦截恶意归档文件。
