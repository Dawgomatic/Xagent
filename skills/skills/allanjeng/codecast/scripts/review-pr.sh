...
*Generated by [codecast](https://github.com/codecast) review mode*" 2>/dev/null
    if [ $? -eq 0 ]; then
      ...
      echo "  Failed to post review comment (check gh auth)" >&2
    fi
  else
    echo "  Review file is empty — skipping comment" >&2
  fi
elif [ "$POST_COMMENT" = true ]; then
  echo "  Review file not found at $REVIEW_OUTPUT — agent may not have written it" >&2
fi
