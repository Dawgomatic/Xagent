...
│  │          AGENT LOOP              │                            │
│  │                                  │                            │
│  │  Context Builder ── LLM Call (RL Proxy)           │
│  │       ▲                  │                         │
│  │       │              ToolCall?                     │
│  │       │              ▼                             │
...
│  │  Sleep Cycle (Self-Improvement)        |  Memory Consolidation│
│  └─────────────────────────────────────────────────────────────┘│
└──────────────────────────────────────────────────────────────────┘
   ...
   │  docker-compose.rl      ── OpenClaw-RL Proxy   │
   └───────────────────────────────────────────────┘
```
...
| Knowledge Vault | `pkg/vault/` | Obsidian-compatible graph memory |
| Orchestration | `pkg/orchestration/` | SWE100821: DAG + roles + aggregation |
| Sandbox | `pkg/sandbox/` | SWE100821: Namespace isolation |
| Sleep | `pkg/agent/sleep.go` | Continuous Improvement / RL Pipeline Trigger |
...
```
               ...
               │  Capture final stats│
               │  Write reflection   │
               │  Trigger Sleep Cycle│── Self-Improvement / RL
...
Context Builder
  ...
  ├─ Load memory (MEMORY.md / Vault)
  └─ Build system prompt
     │
     ▼
LLM Provider / RL Proxy (Port 30000)
     │
...
└── workspace/
    ...
    ├── vault/           ← Obsidian-compatible graph memory (markdown pages)
...
| Knowledge Vault | `pkg/vault/` | Obsidian-compatible graph memory (Markdown based) |
| Sleep Cycle | `pkg/agent/sleep.go` | Scales resource processing, reflection, triggers RL updates |
